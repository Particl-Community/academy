...
.. rst-class:: bignums
     ...
     #. Edit the file and set the correct parameters by modifying the following values to your preferences.
		...
		* :guilabel:`amount_step`: Set a size increment to ensure offers are posted up to the desired `amount`.

		* :guilabel:`min_coin_from_amt`: Reserved balance. Minimum coin balance for the script to automatically post offers.
		...
		* :guilabel:`min_swap_amount`: This refers to the smallest amount of coins a bid must request for the script to automatically accept the offer. Remember that each transaction incurs on-chain transaction fees. Thus, it may be beneficial to set this value higher than the current on-chain fees.
